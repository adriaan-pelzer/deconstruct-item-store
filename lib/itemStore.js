...
const getRedisClient = require ( './getRedisClient.js' );
...
const otherPathsPath = type => ( [ 'typeConfigs', type, 'otherPaths' ] );
const metadataPathsPath = type => ( [ 'typeConfigs', type, 'metadataPaths' ] );
const automaticMetadataPathsPath = type => ( [ 'automaticMetadataPaths' ] );
...
const getRequiredPaths = type => {
    const config = R.head ( configs );

    return R.filter ( path => R.contains ( 'required', path.attrs || [] ), R.reduce ( R.concat, [], [
        R.path ( otherPathsPath ( type ), config ) || [],
        R.path ( metadataPathsPath ( type ), config ) || []
    ] ) );
};

const getLimitedPaths = type => {
    const config = R.head ( configs );

    if ( R.path ( [ 'strictSchema' ], config ) === false || R.isNil ( R.path ( otherPathsPath ( type ), config ) ) ) {
        return null;
    }
    ...
    return R.reduce ( R.concat, [], [
        R.path ( otherPathsPath ( type ), config ) || [],
        R.path ( metadataPathsPath ( type ), config ) || [],
        R.path ( automaticMetadataPathsPath ( type ), config ) || []
    ] );
};

const removeEmptyNodes = O => {
    if ( R.type ( O ) === 'Array' ) {
        if ( R.isEmpty ( O ) ) { return undefined; }
        return R.reject ( R.isNil, R.map ( removeEmptyNodes, O ) );
    }
    if ( R.type ( O ) === 'Object' ) {
        if ( R.isEmpty ( R.keys ( O ) ) ) { return undefined; }
        return R.fromPairs ( R.reject ( ( [ key, value ] ) => R.isNil ( value ), R.zip (
            R.keys ( O ),
            R.map ( removeEmptyNodes, R.values ( O ) )
        ) ) );
    }
    return O;
};

const encapsulateItemRaw = ( type, id, prevItem, item, removeAtomic ) => {
    const config = R.head ( configs );
    const time = new Date ().valueOf ();
    const automaticMetadataPaths = config.automaticMetadataPaths || [];
    const atomicAttrPaths = removeAtomic ? getAttributePaths ( 'atomic', type ) : [];

    const mergedItem = R.reduce (
        ( mergedItem, atomicAttrPath ) => R.dissocPath ( atomicAttrPath.path, mergedItem ),
        R.merge ( R.fromPairs ( R.reject ( R.isNil, R.map (
            pathObj => R.path ( pathObj.path, prevItem ) ? [ R.last ( pathObj.path ), R.path ( pathObj.path, prevItem ) ] : null,
...
const encapsulateItem = ( type, id, prevItem, item ) => encapsulateItemRaw ( type, id, prevItem, item, false );
    ...
    return {
        atomicAdd: ( encapsulatedItem, callback ) => {
            return H ( getAttributePaths ( 'atomic', encapsulatedItem.type ) )
                .filter ( atomicAttributePath => atomicHelper[atomicAttributePath.type].validate (
                    R.path ( [ 'item', ...atomicAttributePath.path ], encapsulatedItem )
                ) )
...
const saveItemRaw = ( stubs, encapsulatedItem, callback ) => {
    ...
    const limitedPaths = getLimitedPaths ( encapsulatedItem.type );
    const illegalObject = limitedPaths === null ? {} : R.reduce ( R.flip ( R.dissocPath ), encapsulatedItem.item, R.map ( R.prop ( 'path' ), limitedPaths ) );
    const illegalKeys = limitedPaths === null ? [] : R.keys ( removeEmptyNodes ( illegalObject ) );
    const requiredButNotPresentPaths = R.filter (
        path => R.isNil ( R.path ( path, encapsulatedItem.item ) ),
        R.map ( R.prop ( 'path' ), getRequiredPaths ( encapsulatedItem.type ) )
    );

    if ( R.not ( R.isEmpty ( illegalKeys ) ) ) {
        return callback ( {
            code: 400,
            message: `The following keys are not allowed in the item payload: ${illegalKeys.join ( ', ' )}`
        } );
    }

    if ( R.not ( R.isEmpty ( requiredButNotPresentPaths ) ) ) {
        return callback ( {
            code: 400,
            message: `The following keys are required in the item payload: ${requiredButNotPresentPaths.map ( R.join ( '.' ) ).join ( ', ' )}`
        } );
    }
...
const saveItem = ( stubs, type, id, prevItem, item, callback ) => {
    return saveItemRaw ( stubs, encapsulateItemRaw ( type, id, prevItem, item, false ), callback );
};

const saveItemNoAtomic = ( stubs, type, id, prevItem, item, callback ) => {
    return saveItemRaw ( stubs, encapsulateItemRaw ( type, id, prevItem, item, true ), callback );
};

const deleteItemRaw = ( stubs, deleteAtomic, type, id, callback ) => {
...
const deleteItem = ( stubs, type, id, callback ) => deleteItemRaw ( stubs, true, type, id, callback );
const deleteItemLeaveAtomic = ( stubs, type, id, callback ) => deleteItemRaw ( stubs, false, type, id, callback );

const getItemScore = ( stubs, type, id, callback ) => {
    ...
    return redisStream ( 'zscore' )( [ `items-${type}`, id ] )
        .flatMap ( H.wrapCallback ( ( score, callback ) => {
            if ( ! score ) {
        ...
        .toCallback ( ( error, result ) => callback ( error, result ) );
};
...
const getItem = ( stubs, type, id, callback ) => {
    ...
    return H.wrapCallback ( getItemScore )( stubs, type, id )
        .flatMap ( score => {
        ...
        .toCallback ( ( error, response ) => callback ( error, response ) );
};

const getItemHistory = ( stubs, type, id, callback ) => {
    const clients = getClients ( stubs, 'getItem' );
    ...
    return H.wrapCallback ( storeItem ( clients.dynamo ).get )( id )
        ...
        .flatMap ( item => {
            if ( item.previous_id ) {
                return H.wrapCallback ( getItemHistory )( stubs, type, item.previous_id )
                    .map ( previous_items => R.concat ( [ item ], previous_items ) );
            }
...
const getItemCount = ( stubs, type, query, callback ) => {
    ...
    const valiDate = ( def, dateString ) => {
        const parsedDate = new Date ( ( R.type ( dateString ) === 'String' && dateString.match ( /^\d+$/ ) ) ? parseInt ( dateString, 10 ) : dateString );
            ...
            return redisStream ( 'zrevrangebyscore' )( [
                ...
                .flatMap ( ids => {
                    if ( itemSet.match ( /^tmp-/ ) ) {
                        return redisStream ( 'delTmp' )( [ itemSet ] )
                            .map ( R.always ( ids ) );
                    }
...
module.exports = {
    ...
    saveItemNoAtomic: R.curry ( saveItemNoAtomic ) /* stubs, type, id, prevItem, item, callback */,
    saveItemRaw: R.curry ( saveItemRaw ) /* stubs, encapsulatedItem, callback */,
    deleteItem: R.curry ( deleteItem ) /* stubs, type, id, callback */,
    deleteItemLeaveAtomic: R.curry ( deleteItemLeaveAtomic ) /* stubs, type, id, callback */,
    getItem: R.curry ( getItem ) /* stubs, type, id, callback */,
    getItemHistory: R.curry ( getItemHistory ) /* stubs, type, id, callback */,
    getItemScore: R.curry ( getItemScore ) /* stubs, type, id, callback */,
