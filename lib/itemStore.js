...
const getRedisClient = stub => {
    const client = stub || redis.createClient ( {
        port: process.env['REDIS_PORT'] || R.path ( [ 'redis', 'port' ], config ) || 6379,
        host: process.env['REDIS_HOST'] || R.path ( [ 'redis', 'host' ], config ) || 'localhost'
    } );
...
const encapsulateItem = ( type, id, prevItem, item ) => {
    ...
    return R.reduce ( R.merge, {}, [
        R.reduce ( ( encapsulatedItem, pathObj ) => {
            ...
            type: type
        }, R.path ( [ 'typeConfigs', type, 'metadataPaths' ], config ) || [] ),
        { item: item },
        prevItem ? {
...
const storeItem = dynamoClient => {
    const storeItemHelper = ( dynamoMethodname, parms, callback ) => {
        return dynamoClient[dynamoMethodname] ( R.merge ( {
            TableName: process.env['DYNAMO_TABLE'] || R.path ( [ 'dynamo', 'table' ], config ) || 'items'
        }, parms ), ( error, data ) => {
            if ( error ) {
...
const saveItem = ( stubs, type, id, prevItem, item, callback ) => {
    const clients = getClients ( stubs, 'saveItem' );

    const encapsulatedItem = encapsulateItem ( type, id, prevItem, item );
