...
const getItemCount = ( stubs, type, query, callback ) => {
    ...
    const sortedSetNames = queryObject.queryObjToSortedSetNames ( type, queryObject.queryToObj ( query ) );
    const keys = R.uniq ( R.reduce ( R.concat, [], R.values ( sortedSetNames ) ) );
    const args = R.map ( R.join ( ',' ), R.values ( sortedSetNames ) );
    const before = valiDate ( '+inf', R.path ( [ 'before' ], query ) );
    const after = valiDate ( '-inf', R.path ( [ 'after' ], query ) );

    return redisStream ( 'runLua' )( {
        scriptName: isGeoQuery ? 'categoryGeoCount' : 'categoryCount',
        keys: isGeoQuery ?
            R.concat ( [ `items-${type}-geo` ], keys ) :
            keys,
        args: isGeoQuery ?
            R.concat ( [ `items-${type}-geo`, query.lng, query.lat, query.radius, query.units || 'mi', before, after ], args ) :
            R.concat ( [ before, after ], args )
    } )
        .toCallback ( callback );
};
...
const getItemsDehydratedWithDistance = ( clients, type, query ) => {
    ...
    return redisStream ( 'runLua' )( {
        scriptName: isGeoQuery ? 'categoryGeo' : 'category',
        keys: isGeoQuery ?
            R.concat ( [ `items-${type}-geo` ], keys ) :
            keys,
        args: isGeoQuery ?
            R.concat ( [ `items-${type}-geo`, query.lng, query.lat, query.radius, query.units || 'mi', offset, count, before, after ], args ) :
            R.concat ( [ offset, count, before, after ], args )
    } )
        .map ( R.splitEvery ( isGeoQuery ? 3 : 2 ) )
        .map ( R.map ( idSet => R.concat ( idSet, [ undefined ] ) ) )
        .sequence ()
        .flatMap ( idTriplet => {
