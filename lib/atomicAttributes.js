...
module.exports = redisClient => {
    ...
    const one = R.compose ( R.head, oneOrMany );
    ...
    const atomicAttributes = {
        key: {
            validate: members => { return atomicAttributes.key.validateSingle ( members ); },
            validateSingle: member => R.type ( member ) === 'String',
            create: ( { parentItemId, attributePath, members }, callback ) => {
                return atomicAttributes.key.set ( { parentItemId, attributePath, members }, callback );
            },
            set: ( { parentItemId, attributePath, members }, callback ) => {
                return redisClient.set ( [ getKey ( parentItemId, attributePath ), members ], callback );
            },
            get: ( { parentItemId, attributePath }, callback ) => {
                return redisClient.get ( [ getKey ( parentItemId, attributePath ) ], callback );
            },
            getAll: ( { parentItemId, attributePath }, callback ) => {
                return atomicAttributes.key.get ( { parentItemId, attributePath }, callback );
            },
            card: ( { parentItemId, attributePath }, callback ) => {
                return H.wrapCallback ( atomicAttributes.key.get )( { parentItemId, attributePath } )
                    .toCallback ( ( error, result ) => {
                        return callback ( null, result ? 1 : 0 );
                    } );
            },
        ...
        list: {
            ...
            trimToRange: ( { parentItemId, attributePath, start, stop }, callback ) => {
                return redisClient.ltrim ( [ getKey ( parentItemId, attributePath ), start, stop ], callback );
            },
            card: ( { parentItemId, attributePath }, callback ) => {
        ...
        zset: {
            ...
            validateSingle: member => R.type ( member ) === 'Object' && R.type ( member.score ) === 'Number' && R.type ( member.member ) === 'String',
            create: ( { parentItemId, attributePath, members }, callback ) => {
...
if ( ! module.parent ) {
    ...
    return test ( 'list', 'create', { members: [ 'first', 'second' ] }, 2 )
        ...
        .flatMap ( R.always ( test ( 'list', 'trimToRange', { start: 1, stop: 3 }, 'OK' ) ) )
        .flatMap ( R.always ( test ( 'list', 'get', { start: 0, stop: -1 }, [ 'quarter', 'first', 'oneAndAHalfth' ] ) ) )
        .flatMap ( R.always ( test ( 'list', 'card', {}, 3 ) ) )
        ...
        .flatMap ( R.always ( test ( 'key', 'card', {}, 0 ) ) )
        .flatMap ( R.always ( test ( 'key', 'create', { members: 'first' }, 'OK' ) ) )
        .flatMap ( R.always ( test ( 'key', 'get', {}, 'first' ) ) )
        .flatMap ( R.always ( test ( 'key', 'getAll', {}, 'first' ) ) )
        .flatMap ( R.always ( test ( 'key', 'card', {}, 1 ) ) )
        .flatMap ( R.always ( test ( 'key', 'set', { members: 'second' }, 'OK' ) ) )
        .flatMap ( R.always ( test ( 'key', 'get', {}, 'second' ) ) )
        .flatMap ( R.always ( test ( 'key', 'getAll', {}, 'second' ) ) )
        .flatMap ( R.always ( test ( 'key', 'card', {}, 1 ) ) )
        .flatMap ( R.always ( test ( 'key', 'delete', {}, 1 ) ) )
        .flatMap ( R.always ( test ( 'key', 'card', {}, 0 ) ) )
