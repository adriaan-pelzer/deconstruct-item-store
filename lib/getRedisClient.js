...
const getRedisConfigs = env => {
    ...
    const read_hosts = ( env.READ_REDIS_HOSTS || env.READ_REDIS_HOSTS_FO || env.PRIMARY_REDIS_HOST_FO ) ? R.map ( R.trim, R.reduce (
        R.concat, [
            env.PRIMARY_REDIS_HOST_FO ? [ env.PRIMARY_REDIS_HOST_FO ] : [],
            env.READ_REDIS_HOSTS.split ( ',' ) || [],
            env.READ_REDIS_HOSTS_FO.split ( ',' ) || []
        ]
    ) ) : [ write_host ];

    const read_ports = ( env.READ_REDIS_PORTS || env.READ_REDIS_PORTS_FO || env.PRIMARY_REDIS_PORT_FO ) ? R.map ( R.trim, R.reduce (
        R.concat, [
            env.PRIMARY_REDIS_PORT_FO ? [ env.PRIMARY_REDIS_PORT_FO ] : [],
            env.READ_REDIS_PORTS.split ( ',' ) || [],
            env.READ_REDIS_PORTS_FO.split ( ',' ) || []
        ]
    ) ) : [ write_port ];
