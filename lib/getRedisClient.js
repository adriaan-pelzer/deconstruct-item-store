...
const getRedisConfigs = env => {
    ...
    const write_host = env[prh] || env.REDIS_HOST || 'localhost';
    const write_port = env[prp] || env.REDIS_PORT || 6379;
    const read_hosts = env[rrh] ? R.map ( R.trim, env[rrh].split ( ',' ) ) : [ write_host ];
    const read_ports = env[rrp] ? R.map ( R.trim, env[rrp].split ( ',' ) ) : [ write_port ];
