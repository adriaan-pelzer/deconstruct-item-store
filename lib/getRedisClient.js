...
const getRedisConfigs = env => {
    ...
    const prh = env.REDIS_FO ? 'PRIMARY_REDIS_HOST_FO' : 'PRIMARY_REDIS_HOST';
    const prp = env.REDIS_FO ? 'PRIMARY_REDIS_PORT_FO' : 'PRIMARY_REDIS_PORT';
    const rrh = env.REDIS_FO ? 'READ_REDIS_HOSTS_FO' : 'READ_REDIS_HOSTS';
    const rrp = env.REDIS_FO ? 'READ_REDIS_PORTS_FO' : 'READ_REDIS_PORTS';
    ...
    const read_hosts = ( env.READ_REDIS_HOSTS || env.READ_REDIS_HOSTS_FO || env.PRIMARY_REDIS_HOST_FO ) ? R.map ( R.trim, R.concat ( env.PRIMARY_REDIS_HOST_FO ? [ env.PRIMARY_REDIS_HOST_FO ] : [], R.concat ( env.READ_REDIS_HOSTS.split ( ',' ) || [], env.READ_REDIS_HOSTS_FO.split ( ',' ) || [] ) ) ) : [ write_host ];

    const read_ports = ( env.READ_REDIS_PORTS || env.READ_REDIS_PORTS_FO || env.PRIMARY_REDIS_PORT_FO ) ? R.map ( R.trim, R.concat ( env.PRIMARY_REDIS_HOST_FO ? [ env.PRIMARY_REDIS_HOST_FO ] : [], R.concat ( env.READ_REDIS_PORTS.split ( ',' ) || [], env.READ_REDIS_PORTS_FO.split ( ',' ) || [] ) ) ) : [ write_port ];
    
    const chosen_read_index = Math.floor ( Math.random () * read_hosts.length );

    if ( read_hosts.length !== read_ports.length ) {
        throw new Error ( `environment variable ${rrh} specifies ${read_hosts.length > read_ports.length ? 'more' : 'fewer'} entries than ${rrp}` );
    }

    const out = { write: { host: write_host, port: write_port }, read: { host: read_hosts[chosen_read_index], port: read_ports[chosen_read_index] } };
    return out;
};
