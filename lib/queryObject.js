...
const queryObjToSortedSetNames = ( type, queryObj ) => {
    ...
    const setNames = R.reduce ( ( setNames, key ) => {
        return R.concat ( setNames, [ [
            randomTempName (),
            R.map ( keyValueToSetName ( key ), R.flatten ( R.of ( queryObj[key] ) ) )
        ] ] );
    }, [], R.keys ( R.omit ( [ 'before', 'after', 'count', 'lat', 'lng', 'radius', 'units', 'offset' ], queryObj ) ) )

    if ( R.isEmpty ( R.values ( setNames ) ) ) {
        return R.fromPairs ( [ [ randomTempName (), [ [ 'items', type ].join ( '-' ) ] ] ] );
    };

    return R.fromPairs ( setNames );
    /*return R.merge ( R.fromPairs ( R.reduce ( ( setNames, key ) => {
        return R.concat ( setNames, [ [
            randomTempName (),
            R.map ( keyValueToSetName ( key ), R.flatten ( R.of ( queryObj[key] ) ) )
        ] ] );
    }, [], R.keys ( R.omit ( [ 'before', 'after', 'count', 'lat', 'lng', 'radius', 'units', 'offset' ], queryObj ) ) ) ), R.fromPairs ( [ [
        randomTempName (),
        [ [ 'items', type ].join ( '-' ) ]
    ] ] ) );*/
};
